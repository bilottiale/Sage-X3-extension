...
export class FourGLDiagnosticsProvider {
  constructor(private parser: FourGLParser) {}
  ...
  private getSemanticDiagnostics(document: TextDocument): Diagnostic[] {
  ...
  private checkUndeclaredVariables(line: string, lineNumber: number, diagnostics: Diagnostic[]): void {
  ...
  private checkSqlSyntax(line: string, lineNumber: number, diagnostics: Diagnostic[]): void {
  ...
  private checkFormSyntax(line: string, lineNumber: number, diagnostics: Diagnostic[]): void {
    const upperLine = line.toUpperCase().trim();
  ...
  private checkDataTypes(line: string, lineNumber: number, diagnostics: Diagnostic[]): void {
  ...
  private checkControlFlow(line: string, lineNumber: number, diagnostics: Diagnostic[]): void {
    const upperLine = line.toUpperCase().trim();
