...
export class SageX3ConnectionProvider {
    private serverUrl: string;
    private folder: string;
    private username: string;
    private password: string;
    private cachedTables: SageX3Table[] | null = null;
  ...
  async testConnection(): Promise<boolean> {
    ...
    async getAvailableFiles(): Promise<string[]> {
        try {
            const response = await this.axiosInstance.get(
                `${this.serverUrl}/${this.folder}/files`,
                {
                    auth: {
    ...
    async getFileContent(filename: string): Promise<string | null> {
        try {
            const response = await this.axiosInstance.get(
                `${this.serverUrl}/${this.folder}/download`,
                {
                    params: { filename },
                    auth: {
                        username: this.username,
   ...
   */
  async getDatabaseSchema(): Promise<SageX3Table[]> {
    try {
      // This endpoint would need to be added to the server
      const response = await this.axiosInstance.get(
        `${this.serverUrl}/${this.folder}/schema`,
        {
          auth: {
            username: this.username,
            password: this.password
      ...
      const tables: SageX3Table[] = response.data.tables || [];
      return tables;
    } catch (error) {
   ...
   */
  async getTableCompletions(prefix: string = ''): Promise<CompletionItem[]> {
   ...
   */
  async getFieldCompletions(tableName: string, prefix: string = ''): Promise<CompletionItem[]> {
  ...
  private getFieldCompletionsFromTable(table: SageX3Table, prefix: string): CompletionItem[] {
    return table.fields
   ...
   */
  async getBracketFieldCompletions(tableName: string, prefix: string = ''): Promise<CompletionItem[]> {
  ...
  private createBracketFieldCompletions(table: SageX3Table, originalTableRef: string, prefix: string): CompletionItem[] {
    return table.fields
      .filter(field => field.name.toLowerCase().startsWith(prefix.toLowerCase()))
      .map(field => ({
        label: field.name,
   ...
   */
  parseTableReference(line: string, position: number): { tableName: string; fieldPrefix: string; isAfterBracket: boolean } | null {
   ...
   */
  private findFullTableName(abbreviation: string): string {
   ...
   */
  private getDefaultTables(): SageX3Table[] {
    return [
      {
        name: 'BPCUSTOMER',
